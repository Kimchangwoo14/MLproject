...
@property NSMutableArray *leftEyePoints;
@property NSMutableArray *rightEyePoints;
@property int leftMaxDistance;
@property int leftMinDistance;
@property int rightMaxDistance;
...
// FP
  ...
  for (FIRVisionPoint *point in leftEyeContour.points) {
    ...
    [self normalizedPointFromVisionPoint:point width:width height:height];
      ...
      [_leftEyePoints addObject:[NSNumber valueWithCGPoint:cgPoint]];
      
       // left Eyes Array add cgPoint
    [UIUtilities addCircleAtPoint:cgPoint
                           toView:self->_annotationOverlayView
  ...
  for (FIRVisionPoint *point in rightEyeContour.points) {
    ...
    [self normalizedPointFromVisionPoint:point width:width height:height];
      ...
      [_rightEyePoints addObject:[NSNumber valueWithCGPoint:cgPoint]];
      // Right Eyes Array add cgPoint
    [UIUtilities addCircleAtPoint:cgPoint
                           toView:self->_annotationOverlayView
                            color:UIColor.cyanColor
                           radius:FIRSmallDotRadius];
...
// FP
  ...
  for (FIRVisionPoint *point in upperLipTopContour.points) {
    ...
    [self normalizedPointFromVisionPoint:point width:width height:height];
      ...
      //Lips Array add cgPoint ?will use 하품
...
-(void) CalLeftLength{
    if([_leftEyePoints count]<=0)return;
    else {
        float MaxY = 0.0;
        float MinY = 0.0;
  
        for(NSNumber *pointNum in _leftEyePoints){
            CGPoint point = [pointNum CGPointValue];
            if(MaxY==0.0) MaxY = point.y;
            else if(point.y>MaxY) MaxY = point.y;
            if(MinY==0.0) MinY = point.y;
            else if(point.y<MinY) MinY = point.y;
        }
       
        float distance = MaxY - MinY;
        if(_leftMaxDistance == 0.0) _leftMaxDistance = distance;
        else if(_leftMaxDistance < distance) _leftMaxDistance = distance;
        if(_leftMinDistance == 0.0 && _leftMaxDistance>distance) _leftMinDistance = distance;
        else if(_leftMinDistance > distance) {
            _leftCloseCount = 0;
...
-(void) CalRightLength{
    if([_rightEyePoints count]<=0)return;
        ...
        float MaxY = 0.0;
        float MinY = 0.0;
        for(NSNumber *pointNum in _rightEyePoints){
            CGPoint point = [pointNum CGPointValue];
            if(MaxY==0.0) MaxY = point.y;
            else if(point.y>MaxY) MaxY = point.y;
            if(MinY==0.0) MinY = point.y;
        ...
        int distance = (int)MaxY - (int)MinY;
        if(_rightMaxDistance == 0.0) _rightMaxDistance = distance;
        else if(_rightMaxDistance < distance) _rightMaxDistance = distance;
        if(_rightMinDistance == 0.0 && _rightMaxDistance>distance) _rightMinDistance = distance;
        else if(_rightMinDistance > distance) {
            _rightCloseCount = 0;
